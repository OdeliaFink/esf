...
if ( ! function_exists( 'kalium_header_top_bar_display' ) ) {
	...
	function kalium_header_top_bar_display() {
		...
		// Content args
		$content_args = apply_filters( 'kalium_header_top_bar_content_args', [
...
if ( ! function_exists( 'kalium_header_menu_bars_button' ) ) {
	...
	function kalium_header_menu_bars_button( $action, $skin = '', $opts = [] ) {
		...
		// Trigger action link
		echo sprintf( '<a href="#" class="%s" aria-label="Toggle navigation" role="button" %s>', kalium()->helpers->list_classes( $classes ), implode( ' ', $atts ) );
        ...
        <div <?php kalium_class_attr( $classes ); ?>>
            <form role="search" method="get" action="<?php echo esc_url( kalium_search_url() ); ?>">
                <div class="search-field">
                    <span><?php esc_html_e( 'SEARCH...', 'kalium' ); ?></span>
                    <input type="search" value="" autocomplete="off" name="s"/>
                </div>

                <div class="search-icon">
                    <a href="#" data-animation="<?php echo $animation; ?>" aria-label="Search site">
				...
				if ( kalium()->request->has( 'lang' ) ) {
					echo sprintf( '<input type="hidden" name="lang" value="%s" />', esc_attr( kalium()->request->query( 'lang' ) ) );
				}
				?>
            </form>
