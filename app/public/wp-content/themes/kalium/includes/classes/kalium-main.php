...
final class Kalium extends Kalium_Base {
	...
	public function admin_init() {
		...
		add_action( 'wp_ajax_kalium_hide_notice', [ $this, 'hide_notice' ] );
	}

	/**
	 * Display notices of the theme.
	 */
	public function display_notices() {
		...
		if ( false === $notices ) {
			...
			set_site_transient( 'kalium_notices', $notices, HOUR_IN_SECONDS * 6 );
		}
		...
		if ( is_array( $notices ) && ! empty( trim( $notices['content'] ) ) && $notices['timestamp'] < time() && ! get_user_meta( get_current_user_id(), 'kalium_notices_hide-' . $notices['id'], true ) ) {
			kalium()->helpers->build_dom_element( [
				'echo'       => true,
				'tag_name'   => 'div',
				'content'    => '<style>' . wp_kses_post( $notices['style'] ) . '</style>' . wp_kses_post( $notices['content'] ),
				'attributes' => [
					'id'    => $notices['id'],
					'class' => [
						'laborator-notice',
	 ...
	 * @return void
	 */
	public function admin_page_theme_options() {
